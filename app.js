...
app.post('/register', (req, res) => {
  const { username, password, email, cuisine, imageUrl } = req.body; // Added imageUrl
  ...
  db.serialize(() => {
    db.run(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `, [username, hashedPassword, email, cuisine, imageUrl], function(err) { // Added imageUrl in the database query
      if (err) {
